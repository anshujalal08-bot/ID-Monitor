...
public partial class MainViewModel : ObservableObject
{
    ...
    public void CheckFirstTimeSetup()
    {
        try
        {
            var needsSetup = _localStorage.NeedsFirstTimeSetup();
            var currentName = _currentWindowsDisplayName.ToLower();
            
            // Also check if username is generic
            if (needsSetup || currentName == "user" || currentName == "admin" || currentName == "administrator")
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    var setupWindow = new Views.FirstTimeSetupWindow(_currentWindowsDisplayName, _currentAutodeskEmail);
                    if (setupWindow.ShowDialog() == true && setupWindow.SetupCompleted)
                    {
                        // Save the profile
                        _localStorage.SaveUserProfileSettings(setupWindow.EnteredName, setupWindow.EnteredEmail);
                        
                        // Update current display name
                        _currentWindowsDisplayName = setupWindow.EnteredName;
                        OnPropertyChanged(nameof(LocalUserDisplayName));
                        
                        // Update email if provided
                        if (!string.IsNullOrEmpty(setupWindow.EnteredEmail))
                        {
                            _currentAutodeskEmail = setupWindow.EnteredEmail;
                        }
                        
                        AddDebug($"First-time setup completed: {setupWindow.EnteredName}");
                    }
                });
            }
        }
        ...
        {
            AddDebug($"First-time setup error: {ex.Message}");
        }
    }
